...
class GDPRCookie
{
    ...
    public function saveChanges($values)
    {
        ...
        foreach ($configUpdates as $name => $value) {
            $this->wpdb->update( 
                $this->wpdb->prefix.'scw_gdprc_config', 
                array('value' => $value), 
                array('name' => $name)
            );
        }
        ...
        foreach ($_POST['cookies'] as $id => $values) {
            $this->wpdb->update( 
                $this->wpdb->prefix.'scw_gdprc_snippets', 
                array(
            ...
            foreach ($values['variables'] as $slug => $value) {
                $this->wpdb->update( 
                    $this->wpdb->prefix.'scw_gdprc_snippet_variables', 
                    array('value' => $value), 
    ...
    {
        $snippetsTable          = $this->wpdb->prefix.'scw_gdprc_snippets';
        $snippetVariablesTable = $this->wpdb->prefix.'scw_gdprc_snippet_variables';
    ...
    {
        $configTable = $this->wpdb->prefix.'scw_gdprc_config';
